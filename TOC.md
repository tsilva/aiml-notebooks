...
### **basics-dimensionality-reduction.ipynb**
Comprehensive introduction to dimensionality reduction techniques for transforming high-dimensional data into lower-dimensional representations. Covers PCA (Principal Component Analysis) for linear dimensionality reduction with explained variance analysis, t-SNE (t-Distributed Stochastic Neighbor Embedding) for non-linear visualization with perplexity tuning, and UMAP (Uniform Manifold Approximation and Projection) for preserving both local and global structure. Demonstrates three key applications: visualization of high-dimensional data (making it human-interpretable), feature extraction for faster training and better generalization, and noise reduction through projection onto principal components. Teaches when to use each method and practical workflows for real-world datasets.
