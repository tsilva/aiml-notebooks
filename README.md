...
### **[mamba1-gentle-intro.ipynb](notebooks/mamba1-gentle-intro.ipynb)** ⭐
A gentle introduction to Mamba for learners coming from RNNs. Builds intuition step-by-step from simple RNN concepts to selective state space models through extensive visualizations and incremental implementation. Covers continuous state space models, discretization with zero-order hold, the selection mechanism (input-dependent parameters), and implements selective scan in three stages (1D → N-dimensional → batched). Each concept is explained with concrete examples and visualizations before code. Perfect first exposure to Mamba before diving into the full implementation.
