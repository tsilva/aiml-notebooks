...
Comprehensive introduction to audio signal processing for machine learning. Covers why audio differs from images and text (requires both time and frequency representations), digital audio basics (sampling rate, bit depth, waveforms), Fourier Transform (converting time domain to frequency domain), Short-Time Fourier Transform/spectrograms (time-frequency representations), mel scale (human perceptual frequency scaling), mel spectrograms, MFCCs (compact decorrelated features for speech), and audio augmentation (time stretching, pitch shifting, noise addition). Builds a simple CNN audio classifier on mel spectrograms, demonstrating the complete pipeline from raw audio to predictions. Compares modern approaches (raw waveform models like WaveNet, self-supervised pre-training with wav2vec/HuBERT, Transformers for audio). Essential foundation for speech recognition, music analysis, and any audio ML task.

### **[speech-recognition.ipynb](notebooks/speech-recognition.ipynb)**
Comprehensive guide to automatic speech recognition (ASR) - converting spoken audio to text. Builds on audio processing fundamentals to teach the complete ASR pipeline: mel spectrograms for feature extraction, the alignment problem (why speech-to-text is hard), CTC (Connectionist Temporal Classification) loss for solving alignment without frame-level labels, and building a working RNN-CTC model from scratch. Demonstrates CTC decoding (greedy vs beam search), evaluation metrics (CER/WER for character and word error rates), and the evolution of ASR systems: classic HMM-GMM → RNN-CTC → attention-based (Listen, Attend, Spell) → self-supervised pretraining (Wav2Vec 2.0). Compares custom models with production systems (Whisper), teaching when to build custom solutions versus using pretrained models. Essential for understanding voice assistants, transcription services, and the intersection of audio and sequence modeling.
...
###  **[basics-mamba-state-space-models.ipynb](notebooks/basics-mamba-state-space-models.ipynb)** ⭐ **Modern Alternative to Transformers**
Comprehensive introduction to State Space Models (SSMs) and Mamba, the groundbreaking linear-complexity alternative to Transformers for long sequence modeling. Teaches continuous-time state space models from control theory, discretization via Zero-Order Hold (ZOH), the recurrence-convolution duality enabling efficient training and inference, and structured state spaces (S4) with HiPPO initialization for long-range dependencies. Demonstrates Mamba's revolutionary innovation: **selective state spaces** where SSM parameters (A, B, C) become input-dependent, enabling content-aware processing like attention but with **linear O(L) complexity** instead of quadratic O(L²). Implements simplified Mamba layer from scratch, empirically verifies linear vs quadratic scaling through benchmarking experiments, trains on sequence copying task showing selective memory capabilities, and provides complete mathematical derivations with visualizations of eigenvalue spectra, convolution kernels, and timescale hierarchies. Explains when to use Mamba (very long sequences >8K, memory-constrained, streaming) vs Transformers (bidirectional context, moderate sequences, abundant compute). Essential for understanding the post-Transformer era of efficient sequence models powering next-generation LLMs.
...
###  **[prompt-engineering-llms.ipynb](notebooks/prompt-engineering-llms.ipynb)** ⭐ **CRITICAL - Programming Large Language Models**
Comprehensive practical guide to prompt engineering, the primary interface for programming and controlling Large Language Models. Teaches foundational techniques: zero-shot vs few-shot prompting (in-context learning), chain-of-thought reasoning ("Let's think step by step" for multi-step problems), and instruction following principles. Covers advanced methods: ReAct (reasoning + acting for tool use), self-consistency (multiple reasoning paths with majority voting), and tree of thoughts (exploring alternatives). Teaches essential design principles: clarity and specificity, delimiters for safety, role assignment, and format specification. Provides reusable prompt templates for common tasks (summarization, extraction, classification), debugging strategies for failed prompts, and complete real-world applications (sentiment analysis, named entity extraction, question answering). Demonstrates all techniques with GPT-2 locally, but principles apply to all LLMs (GPT-4, Claude, Llama). Essential for anyone building LLM applications, as prompt quality directly determines output quality and can improve performance by 20-50% without model changes. The notebook includes 70+ cells with theory before practice, before/after comparisons, and actionable best practices. Note: Best run interactively in Jupyter due to model loading requirements.
...
###  **[continual-learning.ipynb](notebooks/continual-learning.ipynb)** ⭐ **CRITICAL for Lifelong Learning**
Comprehensive guide to continual learning (lifelong learning), the paradigm where models learn sequential tasks without forgetting previous knowledge. Addresses the catastrophic forgetting problem where neural networks dramatically lose performance on old tasks when trained on new ones. Teaches three major solution families: **Replay methods** (storing/generating examples from previous tasks including Experience Replay and Generative Replay), **Regularization methods** (EWC/Elastic Weight Consolidation protecting important weights, online EWC for streaming tasks, SI/Synaptic Intelligence for online weight importance), and **Architecture methods** (Progressive Neural Networks adding new capacity per task, PackNet allocating network subsets). Implements all methods from scratch on split MNIST (sequential digit classification tasks), demonstrating forgetting metrics, forward/backward transfer, and task-incremental vs class-incremental scenarios. Covers evaluation protocols (average accuracy, forgetting measure, intransigence), buffer management strategies for replay, and practical deployment considerations. Shows when to use each approach: replay for best performance with memory budget, regularization for memory-constrained scenarios, architecture for task-known scenarios. Essential for production systems that must adapt to new data distributions, personalized models, robotics (learning new skills), and any application requiring continuous adaptation without retraining from scratch.
